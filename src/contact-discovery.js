...
export function getContactLinkMetadata(href, text, inFooter = false) {
...
export function extractContactsFromText(text, sourceUrl, source, sourceType) {
  ...
  for (const match of value.matchAll(emailPattern)) {
    ...
    const kind = DPO_CONTEXT_PATTERN.test(context) || /^(dpo|datenschutz|data[-_.]?protection)/i.test(localPart)
      ? "dpo"
      : "contact";
    emails.push({
      kind,
